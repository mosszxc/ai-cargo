...
def parse_rate_per_m3(text: str) -> Optional[float]:
    """Parse rate per m³ from text like '320 за куб', '300/м3', '310 м³'."""
    text = text.strip().lower()
    m = re.search(r'(\d+[.,]?\d*)\s*(?:за\s*куб|/м[³3]|м[³3]|за\s*м[³3]|куб)', text)
    if m:
...
def parse_multi_rate_response(text: str) -> list[dict]:
    ...
    for part in parts:
        ...
        if rate is not None:
            ...
            m3 = parse_rate_per_m3(part)
            if m3:
                entry["rate_per_m3"] = m3
...
def process_step(state: dict, message: str) -> dict:
    ...
    if step == "company_name":
        if not message:
            ...
            state["step"] = "routes"
            reply = (
                f"Отлично, **{state['company_name']}**! Настраиваем бот.\n\n"
                "📍 **Шаг 1/5: Маршруты**\n\n"
                "Перечислите откуда и куда возите грузы.\n"
                "Пример: Гуанчжоу→Москва, Суйфуньхэ→Уссурийск, Пекин→Москва"
            )
        ...
        if not routes:
            error = (
                ...
                "Гуанчжоу→Москва, Суйфуньхэ→Уссурийск"
            )
        else:
            state["routes"] = routes
            ...
            reply = (
                ...
                f"🚛 **Шаг 2/5: Транспорт**\n\n"
                f"Какой транспорт на маршруте **{route}**?\n"
                "Напишите через запятую: авто, ЖД, авиа"
            )
        ...
        if not transports:
            error = (
                "Не смог распознать. Напишите через запятую:\n"
                "авто, ЖД, авиа"
            )
        else:
            ...
            if idx + 1 < len(state["routes"]):
                ...
                reply = (
                    f"✅ **{route}**: {transport_names}\n\n"
                    f"Какой транспорт на маршруте **{next_route}**?"
                )
            else:
                ...
                reply = (
                    f"✅ **{route}**: {transport_names}\n\n"
                    f"💰 **Шаг 3/5: Ставки**\n\n"
                    f"У каждого транспорта две ставки:\n"
                    f"• **За кг** — для тяжёлого товара (одежда, запчасти)\n"
                    f"• **За куб** — для лёгкого объёмного (подушки, мячи)\n\n"
                    f"Бот сам определит, что выгоднее, по габаритам товара.\n\n"
                    f"**{first_label}** на **{first_route}**:\n"
                    f"Сколько за кг, за куб (в $) и сроки (дней)?\n"
                    f"Пример: 4.5 за кг, 400 за куб, 30-40 дней"
                )
        ...
        if multi:
            ...
            for item in multi:
                ...
                if t in transports:
                    ...
                    if item.get("rate_per_m3"):
                        entry["rate_per_m3"] = item["rate_per_m3"]
                    state["rates"][route][t] = entry
            ...
            rate_m3 = parse_rate_per_m3(message)
            days_min, days_max = parse_days_range(message)

            if rate is None:
                error = (
                    "Не смог распознать ставку. Напишите так:\n"
                    "4.5 за кг, 400 за куб, 30-40 дней"
                )
            else:
                ...
                if rate_m3:
                    entry["rate_per_m3"] = rate_m3
                state["rates"][route][current_transport] = entry
        ...
        reply = (
            ...
            f"(обычно {DEFAULTS['insurance_pct']}%, напишите своё число или «стандарт»)"
        )
        ...
        reply = (
            ...
            f"📦 **Шаг 5/5: Минимальный вес**\n\n"
            f"Какой минимальный вес для отправки?\n"
            f"(обычно {DEFAULTS['min_weight_kg']} кг, напишите своё число или «стандарт»)"
        )
        ...
        reply = (
            ...
            "Что дальше?\n"
            "• «да» — сохранить\n"
            "• «добавить маршрут» — добавить ещё маршрут\n"
            "• «нет» — начать заново"
        )
        ...
        if text_lower in ("да", "верно", "ок", "ok", "подтверждаю", "всё верно", "все верно", "yes"):
            ...
            reply = "confirmed"
        elif any(kw in text_lower for kw in ("добавить", "новый маршрут", "ещё маршрут", "еще маршрут", "add")):
            # Add route mode — go to routes step but keep existing data
            state["step"] = "add_route"
            reply = (
                "Какой маршрут добавить?\n"
                "Пример: Суйфуньхэ→Уссурийск"
            )
        elif text_lower in ("нет", "не верно", "неверно", "исправить", "no", "заново"):
            ...
            reply = (
                "Напишите «да», «добавить маршрут» или «нет»."
            )

    elif step == "add_route":
        routes = parse_routes(message)
        if not routes:
            error = "Не смог распознать маршрут. Пример: Суйфуньхэ→Уссурийск"
        else:
            # Add new routes to existing ones (skip duplicates)
            for r in routes:
                if r not in state["routes"]:
                    state["routes"].append(r)

            # Collect transports for new routes
            new_routes = [r for r in routes if r not in state["transports"]]
            if new_routes:
                state["_new_routes"] = new_routes
                state["_new_route_idx"] = 0
                state["step"] = "add_route_transport"
                first = new_routes[0]
                reply = (
                    f"✅ Добавлен: {', '.join(new_routes)}\n\n"
                    f"Какой транспорт на маршруте **{first}**?\n"
                    "авто, ЖД, авиа"
                )
            else:
                reply = "Эти маршруты уже есть. Напишите другой или «да» для сохранения."
                state["step"] = "summary"

    elif step == "add_route_transport":
        new_routes = state.get("_new_routes", [])
        idx = state.get("_new_route_idx", 0)
        route = new_routes[idx]
        transports = parse_transports(message)

        if not transports:
            error = "Напишите через запятую: авто, ЖД, авиа"
        else:
            state["transports"][route] = transports
            state["rates"][route] = {}

            if idx + 1 < len(new_routes):
                state["_new_route_idx"] = idx + 1
                next_route = new_routes[idx + 1]
                reply = f"Какой транспорт на маршруте **{next_route}**?"
            else:
                # All new routes have transports, collect rates
                state["step"] = "add_route_rates"
                state["_rate_queue"] = []
                for r in new_routes:
                    for t in state["transports"][r]:
                        state["_rate_queue"].append((r, t))
                state["_rate_queue_idx"] = 0

                r, t = state["_rate_queue"][0]
                label = TRANSPORT_LABELS.get(t, t)
                reply = (
                    f"💰 Ставки для новых маршрутов:\n\n"
                    f"**{label}** на **{r}**:\n"
                    f"За кг, за куб и сроки?\n"
                    f"Пример: 4.5 за кг, 400 за куб, 30-40 дней"
                )

    elif step == "add_route_rates":
        queue = state.get("_rate_queue", [])
        idx = state.get("_rate_queue_idx", 0)
        route, transport = queue[idx]
        ...
        if rate is None:
            error = "Не смог распознать. Пример: 4.5 за кг, 400 за куб, 30-40 дней"
        else:
            entry = {"rate": rate, "days_min": days_min or 15, "days_max": days_max or 25}
            if rate_m3:
                entry["rate_per_m3"] = rate_m3
            if route not in state["rates"]:
                state["rates"][route] = {}
            state["rates"][route][transport] = entry

            if idx + 1 < len(queue):
                state["_rate_queue_idx"] = idx + 1
                r, t = queue[idx + 1]
                label = TRANSPORT_LABELS.get(t, t)
                reply = f"**{label}** на **{r}**:\nЗа кг, за куб и сроки?"
            else:
                # All rates collected, back to summary
                state["step"] = "summary"
                reply = (
                    "✅ Маршруты добавлены!\n\n"
                    + _format_summary(state) + "\n\n"
...
def _advance_rates(state: dict) -> tuple[bool, str]:
    ...
    return True, (
        "✅ Все ставки записаны!\n\n"
        f"🛠 **Шаг 4/5: Доп. услуги**\n\n"
        f"Обрешётка — сколько %?\n"
        f"(обычно {DEFAULTS['crating_pct']}%, напишите своё число или «стандарт»)"
    )


def _format_summary(state: dict) -> str:
    """Format collected data as a readable summary."""
    lines = [f"📋 **Проверьте настройки {state['company_name']}**\n"]
    ...
    for route in state["routes"]:
        ...
        for transport, data in state["rates"].get(route, {}).items():
            ...
            rate = data["rate"]
            rate_m3 = data.get("rate_per_m3")
            days = f"{data['days_min']}–{data['days_max']} дн"
            if rate_m3:
                lines.append(f"  {label} ({days}) — ${rate:g}/кг · ${rate_m3:g}/м³")
            else:
                lines.append(f"  {label} ({days}) — ${rate:g}/кг")
        lines.append("")
...
def generate_rates_json(state: dict) -> dict:
    ...
    for route in state["routes"]:
        ...
        for transport, data in state["rates"].get(route, {}).items():
            ...
            rate_per_m3 = data.get("rate_per_m3")
            transport_entry = {
                "rate_per_kg": rate,
                "days_min": days_min,
                "days_max": days_max,
            ...
            if rate_per_m3:
                transport_entry["rate_per_m3"] = rate_per_m3
            route_data[transport] = transport_entry
...
def edit_state_from_rates(company_id: str, manager_tg_id: str = "") -> dict:
    """Create a wizard state pre-filled from existing rates.json.

    This lets managers re-run the wizard to edit an existing company's config.
    """
    rates_path = get_rates_path(company_id)
    ...
    state["company_name"] = rates.get("company_name", company_id)
    state["crating_pct"] = rates.get("services", {}).get("crating_pct", DEFAULTS["crating_pct"])
    state["insurance_pct"] = rates.get("services", {}).get("insurance_pct", DEFAULTS["insurance_pct"])
    state["min_weight_kg"] = rates.get("min_weight_kg", DEFAULTS["min_weight_kg"])

    # Load routes, transports, and rates
    for route_name, transports in rates.get("routes", {}).items():
        state["routes"].append(route_name)
        state["transports"][route_name] = list(transports.keys())
        state["rates"][route_name] = {}
        for transport_key, config in transports.items():
            entry = {
                "rate": config.get("rate_per_kg", 0),
                "days_min": config.get("days_min", 15),
                "days_max": config.get("days_max", 25),
            }
            if "rate_per_m3" in config:
                entry["rate_per_m3"] = config["rate_per_m3"]
            state["rates"][route_name][transport_key] = entry

    # Jump straight to summary so manager can review and edit
    state["step"] = "summary"
...
def cmd_edit(args):
    """Start wizard pre-filled from existing rates.json."""
    state = edit_state_from_rates(args.company_id, args.manager_tg_id or "")
    if state is None:
        print(json.dumps({
            "ok": False,
            "error": f"Компания {args.company_id} не найдена. Используйте init для новой.",
        }, ensure_ascii=False))
        return
    ...
    print(json.dumps({
        ...
        "step": "summary",
        "reply": (
            f"📝 **Редактирование {state['company_name']}**\n\n"
            f"Текущие настройки:\n\n{summary}\n\n"
            f"Всё верно? Напишите «да» чтобы оставить как есть, "
            f"или «нет» чтобы пройти настройку заново."
        ),
    }, ensure_ascii=False))
...
def main():
    ...
    # edit
    p_edit = subparsers.add_parser("edit", help="Edit existing company config via wizard")
    p_edit.add_argument("company_id", help="Company ID")
    p_edit.add_argument("--manager-tg-id", default="", help="Manager Telegram ID")
