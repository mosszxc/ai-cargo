...
No hardcoded fallback — config.json is the single source of truth.
"""
...
def get_manager_ids(company_id: str) -> set[str]:
    """Get manager Telegram IDs for a company from config.json.

    Returns empty set if config is missing or unreadable (secure default: deny all).
    """
...
def is_manager(caller_id: str, company_id: str) -> bool:
    ...
    if not caller_id:
        return False  # Secure default: no caller_id = deny access (use --no-auth for CLI/test)
